...
def format_elo_rows(
    ...
    *,
    ...
    rank_width = max(len(rank_header), *(len(str(row.rank)) for row in rows)) if include_rank else 0
    ...
    header = f"{player_header.ljust(player_width)} {cp_header.rjust(cp_width)} {wins_header.rjust(wins_width)} {losses_header.rjust(losses_width)} {dodges_header.rjust(dodges_width)}"
    if include_rank:
        header = f"{rank_header.rjust(rank_width)} {header}"
    if include_elo:
        header = f"{player_header.ljust(player_width)} {cp_header.rjust(cp_width)} {elo_header.rjust(elo_width)} {wins_header.rjust(wins_width)} {losses_header.rjust(losses_width)} {dodges_header.rjust(dodges_width)}"
        if include_rank:
            header = f"{rank_header.rjust(rank_width)} {header}"
    if include_scale:
    ...
    for idx, row in enumerate(rows):
        ...
        if include_rank:
            line = f"{str(row.rank).rjust(rank_width)} {line}"
        if include_elo:
            line = (
                f"{row.player.ljust(player_width)} {str(row.cp).rjust(cp_width)} "
